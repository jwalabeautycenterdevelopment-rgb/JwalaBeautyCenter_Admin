...
const AdBanner = () => {
    ...
    const { allBanners, deleteAdSuccessMsg, deleteAdErrorMsg, loadingDelete } = useSelector(
        (state) => state.adBanner
    );
    ...
    useEffect(() => {
        if (deleteAdSuccessMsg) {
            successAlert(deleteAdSuccessMsg);
            dispatch(clearDeleteMsg());
            dispatch(getAdBanner());
        }
        if (deleteAdErrorMsg) {
            errorAlert(deleteAdErrorMsg);
            dispatch(clearDeleteMsg());
        }
    }, [dispatch, deleteAdSuccessMsg, deleteAdErrorMsg]);
    ...
    const handleConfirmDelete = () => {
        dispatch(deleteAdBanner(selectedBannerId));
            ...
            <section className="my-5">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {allBanners?.map((banner) => (
                        <div
                            key={banner?._id}
                            className="bg-white/40 rounded-xl shadow-lg border border-gray-200 p-5 w-full relative group"
                        >
                            ...
                            <div className="flex justify-between items-start">
                                ...
                                <div className="absolute right-0 mt-2 w-32 bg-white border border-gray-200 shadow-md rounded-md py-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none group-hover:pointer-events-auto">
                                    ...
                                    <button
                                        ...
                                        className="w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-100 flex items-center gap-2 text-sm cursor-pointer"
                                    >
                                        <FiEdit /> Update
                                    </button>
                                    ...
                                    <button
                                        ...
                                        className="w-full text-left px-4 py-2 text-red-600 hover:bg-red-100 flex items-center gap-2 text-sm cursor-pointer"
                                    >
                                        <FiTrash2 /> Delete
                                    </button>
